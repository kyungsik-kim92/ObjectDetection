...
### 5. 단어 검색 실패·데이터 없음 예외 처리
검색 실패 또는 로컬에 없는 단어 클릭 시 처리 부재

- **문제:** 단어 검색 API 실패 시나, 데이터에 없는 단어를 눌렀을 때 예외 처리·화면 처리가 없어 크래시나 빈 화면 발생.
- **원인:** 검색 실패·결과 없음·로컬 데이터 검증 로직 미구현.
- **해결:** 단어 검색 실패 시 예외 처리 추가, 데이터에 없는 단어 클릭 시 로컬 데이터 검증 후 분기 처리해 화면 안정성 확보.
...
### 7. KSP와 DataBinding 충돌 (모듈별 빌드 전략)
